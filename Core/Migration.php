...
abstract class Migration
{
    ...
    {
        $columns = is_array($columns) ? implode('`, `', $columns) : $columns;
        return $this->execute("ALTER TABLE `{$tableName}` ADD PRIMARY KEY (`{$columns}`);");
    }

    /**
     * Adiciona uma chave única
     * @param string $tableName Nome da tabela
     * @param string $indexName Nome do índice único
     * @param string|array $columns Coluna ou array de colunas
     * @return bool
     */
    protected function addUniqueKey($tableName, $indexName, $columns)
    {
        $columns = is_array($columns) ? implode('`, `', $columns) : $columns;
        return $this->execute("ALTER TABLE `{$tableName}` ADD UNIQUE KEY `{$indexName}` (`{$columns}`);");
    }

    /** 
     * Adiciona uma chave estrangeira
     */
    protected function addForeignKey($tableName, $columnName, $referencedTable, $referencedColumn, $op)
    {
        return $this->execute("ALTER TABLE `{$tableName}` ADD CONSTRAINT `{$tableName}_{$columnName}_fk` FOREIGN KEY (`{$columnName}`) REFERENCES `{$referencedTable}` (`{$referencedColumn}`) {$op};");
    }
